...
def weighted_total_loss(model, x, t, e, weights):
  # Go through network
  log_sr, log_b, tau = model.forward(x, t)
  log_hr = model.gradient(log_sr, tau, e).log()
  ...
  mask0 = (e == 0)
  if mask0.any():
    error -= (torch.logsumexp(log_balance_sr[mask0], dim=1) * weights[mask0]).sum()
  for k in range(model.risks):
    maskk = (e == (k + 1))
    if maskk.any():
      error -= ((log_balance_sr[maskk, k] + log_hr[maskk]) * weights[maskk]).sum()
