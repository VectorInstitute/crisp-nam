...
def fine_gray_negative_log_likelihood(
    ...
    for k in range(1, num_competing_risks + 1):
        ...
        for i in range(batch_size):
            if event_mask[i]:
                # Standard risk set: still event-free and under observation at t_i
                ...
                for j in range(1, num_competing_risks + 1):
                    if j != k:
                        already_had_competing_event = (events == j) & (
                            times <= times[i]
                        )
                        at_risk = at_risk | already_had_competing_event

                # Calculate log sum of exp of risk scores in risk set
                risk_set_scores = risk_k[at_risk]
